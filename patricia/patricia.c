...
//Bala
#define DATA (0x0A0A0A01)
#define BALA 1

static unsigned int nexthop = DATA;
...
patricia_remove (patricia_tree_t *patricia, patricia_node_t *node)
{
	...
	fprintf (stderr, "patricia_remove: #0 %s/%d (r & l)\n", 
		 prefix_toa (node->prefix), node->prefix->bitlen);
#endif /* PATRICIA_DEBUG */
//Bala added below to check parent
#if 0
	if ((parent) && (parent->prefix)) {
	fprintf (stderr, "patricia_remove: #0 parent %s/%d (r & l)\n", 
		 prefix_toa (parent ->prefix), parent->prefix->bitlen);
	}
	else {
	fprintf (stderr, "patricia_remove: #0 parent not found (r & l)\n");
	}
#endif
...
#if 0
	if ((parent) && (parent->prefix)) {
	fprintf (stderr, "patricia_remove: #1 parent %s/%d (!r & !l)\n", 
		 prefix_toa (parent ->prefix), parent->prefix->bitlen);
	}
	else {
	fprintf (stderr, "patricia_remove: #1 parent not found (!r & !l)\n");
	}
#endif
	Deref_Prefix (node->prefix);
	Delete (node);
...
#if 0
	if ((parent) && (parent->prefix)) {
	fprintf (stderr, "patricia_remove: #2 parent %s/%d (r & l)\n", 
		 prefix_toa (parent ->prefix), parent->prefix->bitlen);
	}
	else {
	fprintf (stderr, "patricia_remove: #2 parent not found (r & l)\n");
	}
#endif
...
#ifdef BALA
        printf ("try_search_exact for %s: %s/%d nh: %x  found\n", string,
	        prefix_toa (node->prefix), node->prefix->bitlen, node->data);
#else
...
#ifdef BALA
        printf ("try_search_best for %s: %s/%d nh: %x  found\n", string,
	        prefix_toa (node->prefix), node->prefix->bitlen, node->data);
#else
