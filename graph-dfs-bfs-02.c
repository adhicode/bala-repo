...
add_edge(graph *g, vertex *x, vertex *y, bool directed)
{
    // MAX degrees check 

    // if bidirected, max connections both ways

    // if directed, x to y edge only created
	...
	while (front != 0) {
		...
		while (p != 0) {
			...
			if (w->mark == 0) {
				...
				visit (w);
				// add w to queue
				nextQueue = (Edge*)malloc(sizeof(Edge));
				nextQueue->vertex = w;
				nextQueue->next = 0;
				rear->next = nextQueue;
				rear = nextQueue;
